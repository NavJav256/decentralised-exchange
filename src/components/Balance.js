...
import navT from '../assets/navT.svg'
...
const Balance = () => {
    ...
    useEffect(() => {
        if(exchange && tokens[0] && tokens[1] && account) loadBalances(exchange, tokens, account, dispatch)
    ...
    const amountHandler = (e, token) => {
        if(token.address === tokens[0].address) setToken1TransferAmount(e.target.value)
            ...
            <div className='exchange__transfers--form'>
                <div className='flex-between'>
                    <p>
                        <small>Token</small>
                        <br/>
                        <img src={navT} width={20} height={21} alt='Token logo' />
                        {symbols && symbols[0]}
                    </p>
                    <p>
                        <small>Wallet</small>
